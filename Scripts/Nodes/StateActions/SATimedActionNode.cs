...
namespace NASB_Moveset_Editor.StateActions
{
	public class SATimedActionNode : StateActionNode
	{
		...
		[Output(connectionType = ConnectionType.Override)] public FloatSourceContainer Time;
		public bool Repeat;
		...
		public int SetData(SATimedAction data, MovesetGraph graph, string assetPath, UnityEngine.Vector2 nodeDepthXY)
		{
			...
			FloatSourceContainerNode node_Time = graph.AddNode<FloatSourceContainerNode>();
			GetPort("Time").Connect(node_Time.GetPort("NodeInput"));
			AssetDatabase.AddObjectToAsset(node_Time, assetPath);
			variableCount += node_Time.SetData(Time, graph, assetPath, nodeDepthXY + new UnityEngine.Vector2(1, variableCount));
			++variableCount;
		...
		public new SATimedAction GetData()
		{
			...
			objToReturn.TID = TypeId.SATimedAction;
			if (GetPort("Time").ConnectionCount > 0)
			{
				FloatSourceContainerNode FloatSourceContainer_Node = (FloatSourceContainerNode)GetPort("Time").GetConnection(0).node;
				objToReturn.Time = FloatSourceContainer_Node.GetData();
			}
