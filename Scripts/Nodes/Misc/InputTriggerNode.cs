...
using static MovesetParser.Misc.InputTrigger;
...
namespace NASB_Moveset_Editor.Misc
{
	public class InputTriggerNode : BaseMovesetNode
	{
		...
		public int SetData(InputTrigger data, MovesetGraph graph, string assetPath, UnityEngine.Vector2 nodeDepthXY)
		{
			name = NodeEditorUtilities.NodeDefaultName(typeof(InputTrigger));
			position.x = nodeDepthXY.x * Consts.NodeXOffset;
			position.y = nodeDepthXY.y * Consts.NodeYOffset;
			int variableCount = 0;
			
			SniffFrames = data.SniffFrames;
			BlockedByEvent = data.BlockedByEvent;
			AddEventOnTrigger = data.AddEventOnTrigger;
			Validator = data.Validator;
			
			InputValidatorNode node_Validator = graph.AddNode<InputValidatorNode>();
			GetPort("Validator").Connect(node_Validator.GetPort("NodeInput"));
			AssetDatabase.AddObjectToAsset(node_Validator, assetPath);
			variableCount += node_Validator.SetData(Validator, graph, assetPath, nodeDepthXY + new UnityEngine.Vector2(1, variableCount));
			++variableCount;
		...
		public InputTrigger GetData()
		{
			...
			objToReturn.BlockedByEvent = BlockedByEvent;
			objToReturn.AddEventOnTrigger = AddEventOnTrigger;
			if (GetPort("Validator").ConnectionCount > 0)
			{
				InputValidatorNode InputValidator_Node = (InputValidatorNode)GetPort("Validator").GetConnection(0).node;
				objToReturn.Validator = InputValidator_Node.GetData();
			}
